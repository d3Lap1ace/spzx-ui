        ...
        >
            ...
            <el-table-column label="轮播图" #default="scope" width="200">
                <div style="height: 50px; float: left">
                    <img
                        v-for="(item, index) in scope.row.sliderUrlList"
                        :key="index"
                        :src="item"
                        width="50"
                    />
        ...
        <el-dialog :title="title" v-model="open" width="60%" append-to-body>
            ...
            >
                ...
                <div v-if="activeIndex == 3">
                    <div style="font-size: large; margin: 30px 30px">
                        提交成功
                    </div>
                </div>
            </el-form>
...
function handleAudit(id, status) {
    const msg =
        status == 1
            ? '是否确认审批通过商品编号为"' + id + '"的数据项？'
            : '是否确认审批驳回商品编号为"' + id + '"的数据项？';
    proxy.$modal
        .confirm(msg)
        .then(function () {
...
const handleRemove = (uploadFile, uploadFiles) => {
    ...
    form.value.sliderUrlList.splice(
        form.value.sliderUrlList.indexOf(uploadFile.url),
        1
    );
